...
```php
voidZpl($void_input_body): \StripyHorse\Model\VoidOutputBody
```
...
$void_input_body = new \StripyHorse\Model\VoidInputBody(); // \StripyHorse\Model\VoidInputBody

try {
    $result = $apiInstance->voidZpl($void_input_body);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ConvertApi->voidZpl: ', $e->getMessage(), PHP_EOL;
}
```
...
| Name | Type | Description  | Notes |
| ------------- | ------------- | ------------- | ------------- |
| **void_input_body** | [**\StripyHorse\Model\VoidInputBody**](../Model/VoidInputBody.md)|  | |

### Return type

[**\StripyHorse\Model\VoidOutputBody**](../Model/VoidOutputBody.md)
